<template>
  <div>
    ...
    <HelpBetter />
    <readme
      title="Онлайн калькулятор расчета объема бетона ленты"
      text="Если вы не знаете, какой объем бетона заказать для заливки ленточного фундамента, воспользуйтесь нашим калькулятором. Если ваш фундамент имеет простую форму, то потребуется указать лишь длину, ширину и высоту. Если же геометрия более сложная, то укажите габариты каждой секции. После ввода всех необходимых данных, кликните на кнопку “Итого”, и калькулятор покажет, сколько бетона нужно для заливки вашего ленточного фундамента."
    />
    <modal
      :modal="modal"
